-- Sprint 7 narrow repair: qualify table references inside resolve_care_circle_invitation.
-- The prior deny/expire/accept updates used unqualified columns that collide with
...
begin
  ...
  if p_action = 'deny' then
    update public.invitations i
    set status = 'declined',
      invited_user_id = coalesce(i.invited_user_id, v_app_user_id)
    where i.id = v_invitation.id
      and i.status = 'pending';
