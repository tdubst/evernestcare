...
const SECTIONS = [
  { id: "documents", label: "Documents", icon: FileText, count: 23, tone: "bg-sky text-sky-foreground" },
  { id: "imaging", label: "Imaging", icon: ScanLine, count: 4, tone: "bg-blush text-blush-foreground" },
  { id: "labs", label: "Labs", icon: FlaskConical, count: 12, tone: "bg-sage text-sage-foreground" },
  { id: "insurance", label: "Insurance", icon: ShieldCheck, count: 3, tone: "bg-sand text-sand-foreground" },
  { id: "medications", label: "Medications", icon: Pill, count: 8, tone: "bg-blush text-blush-foreground" },
  { id: "directives", label: "Directives", icon: ScrollText, count: 2, tone: "bg-sky text-sky-foreground" },
];

const IMAGING = [
  { id: "1", type: "Brain MRI", area: "Head", date: "May 12, 2026", provider: "Dr. Patel · Neurology", summary: "Findings shared with care team. Follow-up scheduled in 3 days.", followup: "Follow-up May 29", tone: "bg-sky text-sky-foreground" },
  { id: "2", type: "Chest X-ray", area: "Chest", date: "Apr 02, 2026", provider: "Mercy Imaging Center", summary: "Routine check. No new action recommended by provider.", followup: "No follow-up needed", tone: "bg-sage text-sage-foreground" },
  { id: "3", type: "Echocardiogram", area: "Heart", date: "Mar 18, 2026", provider: "Dr. Okafor · Cardiology", summary: "Stable function. Recheck in 6 months.", followup: "Recheck Sep 2026", tone: "bg-blush text-blush-foreground" },
  { id: "4", type: "Hip CT", area: "Hip", date: "Feb 04, 2026", provider: "Mercy Imaging Center", summary: "Pre-op imaging archived for orthopedic team.", followup: "Shared with Dr. Lin", tone: "bg-sand text-sand-foreground" },
];
    ...
    return (
      <div>
        <header className="px-6 pt-14 pb-3">
          <button onClick={() => setSection(null)} className="inline-flex items-center gap-1 text-[14px] text-primary mb-3">
            <ChevronLeft className="h-4 w-4" /> Vault
          </button>
          <h1 className="text-[28px] font-semibold tracking-tight">Imaging</h1>
          <p className="text-[13px] text-muted-foreground mt-1">{IMAGING.length} studies · organized by body area</p>
        </header>
        ...
        <section className="px-6 mt-5 space-y-3">
          {filtered.map((s) => (
            <button key={s.id} onClick={() => setImagingId(s.id)} className="w-full flex items-center gap-3 card-soft p-4 text-left">
              <span className={`inline-flex h-14 w-14 items-center justify-center rounded-2xl ${s.tone}`}>
  ...
  return (
    <div>
      <header className="px-6 pt-14 pb-2">
        ...
        <p className="text-[13px] text-muted-foreground mt-1">Margaret's medical records, calmly organized.</p>
        <div className="mt-4 flex items-center gap-2 rounded-full bg-secondary px-4 py-2.5">
          <Search className="h-4 w-4 text-muted-foreground" />
          <input placeholder="Search documents, providers, labs" className="bg-transparent outline-none text-[14px] flex-1" />
        </div>
      </header>
      ...
      <section className="px-6 mt-7">
        <h2 className="text-[13px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">Recently added</h2>
        <div className="card-soft divide-y hairline overflow-hidden">
          {[
            { i: ScanLine, t: "Brain MRI", s: "Dr. Patel · May 12", tone: "bg-sky text-sky-foreground" },
            { i: FlaskConical, t: "Lipid panel", s: "Quest · May 08", tone: "bg-sage text-sage-foreground" },
            { i: FileText, t: "Discharge summary", s: "Mercy Hospital · Apr 28", tone: "bg-sand text-sand-foreground" },
          ].map((r) => (
            <div key={r.t} className="flex items-center gap-3 px-4 py-3.5">
              <span className={`inline-flex h-10 w-10 items-center justify-center rounded-full ${r.tone}`}>
...
function ImagingDetail({ study, onBack }: { study: typeof IMAGING[number]; onBack: () => void }) {
  return (
    <div>
      <header className="px-6 pt-14 pb-3">
        <button onClick={onBack} className="inline-flex items-center gap-1 text-[14px] text-primary mb-3">
      ...
      <div className="px-6">
        <div className="card-soft overflow-hidden">
          ...
          <div className="p-5">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{study.area}</p>
