...
The machine-readable launch approval must name that same full reviewed `main` SHA and the complete committed migration range from the first migration through the current latest migration. The release workflow passes the requested SHA into the approval validator and fails closed if the record targets a different commit, omits either migration boundary, or includes incomplete evidence or owner decisions.
